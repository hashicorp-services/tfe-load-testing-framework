...
"""
TFE Run Operations Load Test

This load test scenario focuses on Terraform Enterprise run operations:
- Creating and uploading configuration versions
- Triggering runs (plan and apply)
- Monitoring run status
- Handling run queue depth
- Run cancellations
...
class TFERunUser(HttpUser):
    """
    Simulates a user performing run operations in TFE.
    
    Task weights determine the frequency of each operation:
    - create_and_trigger_run: 10 (most common - creating and running configs)
    - monitor_run_status: 8 (frequently checking run status)
    - queue_multiple_runs: 3 (occasionally queuing multiple runs)
    - cancel_run: 2 (occasionally canceling runs)
    """
    ...
    def on_start(self):
        ...
        workspace_name = f"loadtest-runs-{uuid.uuid4().hex[:8]}"
...
"""
    ...
    def monitor_run_status(self):
        ...
        try:
            ...
            with self.client.get(
                ...
                # If run is completed, remove from tracking
    ...
    def cancel_run(self):
        ...
        try:
            # Pick a random run to cancel
            run_id = random.choice(self.created_runs)
            
            # Check if run is in a cancelable state
            with self.client.get(
                f"/api/v2/runs/{run_id}",
                headers={
                    "Authorization": f"Bearer {self.tfe.token}",
            ...
            if status in ['pending', 'plan_queued', 'planning', 'planned', 'cost_estimating', 
                ...
                with self.client.post(
                    ...
                    response.success()
                    print(f"Canceled run {run_id} (was {status})")
                    self.created_runs.remove(run_id)
            else:
                print(f"Run {run_id} not in cancelable state: {status}")
...
def on_test_start(environment, **kwargs):
    ...
    print("\n" + "="*80)
    print("TFE Run Operations Load Test Starting")
    print("="*80)
...
def on_test_stop(environment, **kwargs):
    ...
    print("\n" + "="*80)
    print("TFE Run Operations Load Test Completed")
    print("="*80)
    ...
    print("  - Configuration upload performance")
    print("="*80 + "\n")
